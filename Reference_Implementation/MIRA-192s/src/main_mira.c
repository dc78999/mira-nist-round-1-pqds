...
int main(void) {
    ...
    unsigned long long mira192s_mlen = 21;
...
//    (void)syscall(SYS_getrandom, seed, 48, 0);
    ...
    printf("MIRA-192S keypair generation:\n");
    ...
    getrusage(RUSAGE_SELF, &usage_before);
    t1 = cpucyclesStart();
    unsigned char *mout = malloc(mira192s_smlen);
    unsigned long long mout_len;

    t5 = cpucyclesStart();
    if (crypto_sign_open(mout, &mout_len, mira192s_sm, mira192s_smlen, mira192s_pk) == -1) {
        printf("\nFailed\n\n");
        return -1;
    }
    t6 = cpucyclesStop();
